...
class SavedPDFDocument(models.Model):
    user = models.ForeignKey(User)
    pdfdocument = models.ForeignKey(PDFDocument)
    saved_at = models.DateTimeField(auto_now_add=True, editable=False)

    class Meta:
        unique_together = (('user', 'pdfdocument'),)

    def __unicode__(self):
        return '%s %s' %(self.user, self.pdfdocument)
...
def check_userprofile_details(sender, request, user, **kwargs):
    ...
    category_list = CategoryFollow.objects.filter(user=user)
    if not category_list:
        request.session['no_category'] = True
