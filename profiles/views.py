...
from .models import UserProfile, SavedPDFDocument, CategoryFollow, Project
...
class UserInfoView(SetHeadlineMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        saved_pdfdocuments = SavedPDFDocument.objects.filter(user=user)
        categories_follow = CategoryFollow.objects.filter(user=user)
        context['userprofile'] = userprofile
        context['saved_documents'] = saved_pdfdocuments
        context['categories_follow'] = categories_follow
        return context
...
class UserPDFDocumentsView(SetHeadlineMixin, UserInfoMixin, ListView):
    context_object_name = 'pdfdocuments'
    template_name = 'profiles/user_pdfdocuments.html'
    paginate_by = 12

    def get_queryset(self):
        user = get_object_or_404(User, username=self.kwargs['username'])
        self.headline = unicode(user.username) + ' shared PDFDocuments'
        return user.pdfdocument_set.all()
...
class MyPDFDocumentsView(LoginRequiredMixin, RedirectView):
    permanent = False

    def get_redirect_url(self):
        user = self.request.user
        return reverse('user_pdfdocuments', kwargs={'username':user.username})
